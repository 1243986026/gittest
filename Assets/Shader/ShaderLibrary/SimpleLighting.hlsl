#ifndef UNIVERSAL_SIMPLE_LIGHTING_INCLUDED//修改头文件
#define UNIVERSAL_SIMPLE_LIGHTING_INCLUDED
    ...
    #include "../ShaderLibrary/SimpleSurfaceData.hlsl"//修改SurfaceData.hlsl路径
    ...
    //直接光漫反射 直接光高光 
    //间接光漫反射 LightMap Light Probe 
    //间接光环境反射 Reflection Probe
    half DirectBRDFSpecular(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS) //GGX高光
    {
    ...
    half3 GlossyEnvironmentReflection(half3 reflectVector, half perceptualRoughness, half occlusion, SurfaceData surface)
    {
            ...
            half4 encodedIrradiance = SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflectVector, mip);
                ...
                irradiance = lerp(irradiance, irradiance * surface.nightLightData.a, surface.nightLightData.r);
            #endif
    ...
    half3 normalWS, half3 viewDirectionWS, SurfaceData surface)
    {
        half3 reflectVector = reflect(-viewDirectionWS, normalWS);
        half NoV = saturate(dot(normalWS, viewDirectionWS));
        half fresnelTerm = Pow4(1.0 - NoV);
        bakedGI = (sqrt(bakedGI)) * surface.GIColor;
        //half3(0.85, 0.95, 1.0)
        half3 indirectDiffuse = bakedGI * occlusion;
        half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfData.perceptualRoughness, occlusion, surface);
    ...
    half3 GlobalIllumination(BRDFData brdfData, half3 bakedGI, half occlusion, half3 normalWS, half3 viewDirectionWS, SurfaceData surface)
    {
        const BRDFData noClearCoat = (BRDFData)0;
        return GlobalIllumination(brdfData, noClearCoat, 0.0, bakedGI, occlusion, normalWS, viewDirectionWS, surface);
    }
    ...
    half clearCoatMask, bool specularHighlightsOff)
    {
        ...
        half3 brdf = brdfData.diffuse;//漫反射
        #ifndef _SPECULARHIGHLIGHTS_OFF
            [branch] if (!specularHighlightsOff)
            {
    ...
    half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
    {
        ...
        inputData.bakedGI = lerp(inputData.bakedGI * surfaceData.nightLightData.b, inputData.bakedGI * surfaceData.nightLightData.g, surfaceData.nightLightData.r);
        
        MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI);
        half3 color = GlobalIllumination(brdfData, brdfDataClearCoat, surfaceData.clearCoatMask,
        inputData.bakedGI, surfaceData.occlusion,
        inputData.normalWS, inputData.viewDirectionWS, surfaceData); //全局GI 间接光
        color += LightingPhysicallyBased(brdfData, brdfDataClearCoat, //直接光计算
        mainLight,
